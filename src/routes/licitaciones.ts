...
type LicitacionEstado = 'activa' | 'presentada' | 'ganada' | 'perdida' | 'sin_interes' | 'cancelada';
...
router.post('/', [
  ...
  authenticateToken,
  requireManager
], asyncHandler(async (req: Request<object, object, CreateLicitacionBody>, res: Response): Promise<void> => {
...
router.put('/:id/estado', [
  ...
  authenticateToken,
  requireManager
], asyncHandler(async (req: Request<{ id: string }, object, { estado_licitacion: LicitacionEstado; resultado?: string }>, res: Response): Promise<void> => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    res.status(400).json({
      success: false,
