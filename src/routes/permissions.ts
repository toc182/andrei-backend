...
router.get('/users', asyncHandler(async (_req: Request, res: Response): Promise<void> => {
...
router.get('/:userId', asyncHandler(async (req: Request<{ userId: string }>, res: Response): Promise<void> => {
  ...
  if (result.rows.length === 0) {
    res.status(404).json({ success: false, message: 'Permisos no encontrados' });
    return;
  }
...
router.put('/:userId', asyncHandler(async (req: Request<{ userId: string }>, res: Response): Promise<void> => {
  const { userId } = req.params;
...
router.get('/:userId/projects', asyncHandler(async (req: Request<{ userId: string }>, res: Response): Promise<void> => {
  const { userId } = req.params;
...
router.put('/:userId/projects', asyncHandler(async (req: Request<{ userId: string }>, res: Response): Promise<void> => {
  const { userId } = req.params;
  const { projectIds } = req.body as { projectIds: number[] };

  if (!Array.isArray(projectIds)) {
    res.status(400).json({ success: false, message: 'projectIds debe ser un array' });
    return;
  }
