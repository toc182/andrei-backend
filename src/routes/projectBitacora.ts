...
router.get('/:entryId', authenticateToken, asyncHandler(async (req: Request<{ entryId: string }>, res: Response): Promise<void> => {
...
router.post('/projects/:projectId', authenticateToken, upload.array('fotos', 10), asyncHandler(async (req: Request<{ projectId: string }, object, CreateEntryBody>, res: Response): Promise<void> => {
  const { projectId } = req.params;
...
router.put('/:entryId', authenticateToken, asyncHandler(async (req: Request<{ entryId: string }, object, CreateEntryBody>, res: Response): Promise<void> => {
  const { entryId } = req.params;
  const { titulo, contenido } = req.body;
  const userId = req.user!.id;
  ...
  if (existing.rows[0].created_by !== userId) {
    res.status(403).json({ success: false, message: 'No puedes editar esta entrada' });
    return;
  }
...
router.delete('/:entryId', authenticateToken, asyncHandler(async (req: Request<{ entryId: string }>, res: Response): Promise<void> => {
  const { entryId } = req.params;
  const userId = req.user!.id;
  ...
  if (existing.rows[0].created_by !== userId) {
    res.status(403).json({ success: false, message: 'No puedes eliminar esta entrada' });
    return;
  }
...
router.post('/:entryId/comments', authenticateToken, upload.array('fotos', 5), asyncHandler(async (req: Request<{ entryId: string }, object, CommentBody>, res: Response): Promise<void> => {
  const { entryId } = req.params;
...
router.post('/:entryId/attachments', authenticateToken, upload.array('fotos', 10), asyncHandler(async (req: Request<{ entryId: string }>, res: Response): Promise<void> => {
  const { entryId } = req.params;
  const userId = req.user!.id;
  ...
  if (entryCheck.rows[0].created_by !== userId) {
    res.status(403).json({ success: false, message: 'No puedes agregar fotos a esta entrada' });
    return;
  }
...
router.delete('/attachments/:attachmentId', authenticateToken, asyncHandler(async (req: Request<{ attachmentId: string }>, res: Response): Promise<void> => {
  ...
  if (attachment.rows[0].created_by !== userId) {
    res.status(403).json({ success: false, message: 'No puedes eliminar este archivo' });
    return;
  }
