...
router.post('/', authenticateToken, checkPermission('equipos_agregar'), [
  ...
  body('valor_actual').optional({ nullable: true, checkFalsy: true }).isDecimal(),
  ...
  const {
    ...
    costo, valor_actual, rata_mes, proyecto, responsable, estado, observaciones, owner
  } = req.body;
  ...
  const result = await query<{ id: number }>(`
    ...
    RETURNING id
  `, [
    codigo || null, descripcion, marca, modelo, ano, motor || null, chasis || null,
    costo ? parseFloat(String(costo)) : null, valor_actual ? parseFloat(String(valor_actual)) : null,
    rata_mes ? parseFloat(String(rata_mes)) : null, proyecto || null, responsable || null,
...
router.put('/:id', authenticateToken, checkPermission('equipos_editar'), [
  ...
  body('ano').isInt({ min: 1900, max: 2030 }).withMessage('Año debe ser un número válido entre 1900 y 2030'),
  body('owner').isIn(['Pinellas', 'COCP']).withMessage('Owner debe ser Pinellas o COCP')
], asyncHandler(async (req: Request<{ id: string }, object, CreateEquipoBody>, res: Response): Promise<void> => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    res.status(400).json({ success: false, message: 'Datos de entrada inválidos', errors: errors.array() });
    return;
  ...
  const existsResult = await query<{ id: number }>('SELECT id FROM equipos WHERE id = $1 AND activo = true', [id]);
...
router.put('/:id/status', authenticateToken, checkPermission('equipos_editar'), [
  ...
  const { id } = req.params;
  const { estado, proyecto, responsable, rata_mes, observaciones_status } = req.body;
  ...
  await query(`
    ...
    WHERE id = $6
  `, [
    estado || null, proyecto || null, responsable || null,
