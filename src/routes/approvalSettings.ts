...
router.put('/project/:projectId', [
  param('projectId').isInt(),
  body('approvers').isArray().withMessage('Se requiere un array de aprobadores')
], asyncHandler(async (req: Request<{ projectId: string }>, res: Response): Promise<void> => {
  ...
  const project = await query('SELECT id FROM proyectos WHERE id = $1', [projectId]);
  ...
  res.json({ success: true, message: 'Aprobadores actualizados', approvers: result.rows });
}));
