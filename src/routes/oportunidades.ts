...
type OportunidadEstado = 'prospecto' | 'calificada' | 'propuesta' | 'negociacion' | 'cerrada' | 'perdida';
...
router.post('/', [
  ...
  authenticateToken
], asyncHandler(async (req: Request<object, object, CreateOportunidadBody>, res: Response): Promise<void> => {
...
router.put('/:id/estado', [
  ...
  authenticateToken
], asyncHandler(async (req: Request<{ id: string }, object, { estado_oportunidad: OportunidadEstado; notas_comerciales?: string }>, res: Response): Promise<void> => {
  const { id } = req.params;
