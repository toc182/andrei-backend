  ...
  asyncHandler(
    async (req: Request<{ id: string }>, res: Response): Promise<void> => {
      ...
      if (result.rows.length === 0) {
        res
          .status(404)
          .json({ success: false, error: 'Caja menuda no encontrada' });
        return;
      }
...
router.put(
  ...
  asyncHandler(
    async (req: Request<{ id: string }>, res: Response): Promise<void> => {
      ...
      const filesMap = (req.files || {}) as Record<
        string,
        Express.Multer.File[]
      >;
      ...
      if (
        ...
        res
          .status(400)
          .json({ success: false, error: 'Estado debe ser abierta o cerrada' });
        return;
      }
      ...
      if (aperturaFile) {
        ...
        if (prevKey) {
          try {
            ...
            console.error(
              'Error deleting previous opening comprobante:',
              prevKey,
              err,
            );
          }
...
router.post(
  ...
  asyncHandler(
    async (req: Request<{ id: string }>, res: Response): Promise<void> => {
      ...
      const { id } = req.params;
      const { fecha, proveedor, descripcion, monto, itbms, monto_total } =
        req.body;
      const user = req.user!;

      // Verify caja exists and is open
      const caja = await query<{ estado: string }>(
        'SELECT estado FROM cajas_menudas WHERE id = $1',
        [id],
      );
      ...
      if (caja.rows[0].estado !== 'abierta') {
        res
          .status(400)
          .json({ success: false, error: 'La caja menuda está cerrada' });
        return;
      }
...
router.put(
  ...
  asyncHandler(
    async (
      req: Request<{ id: string; gastoId: string }>,
      res: Response,
    ): Promise<void> => {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
      ...
      if (gasto.rows[0].solicitud_reembolso_id !== null) {
        res.status(400).json({
          success: false,
          error: 'No se puede editar un gasto ya reembolsado',
        });
        return;
      }
      ...
      if (fecha !== undefined) {
        sets.push(`fecha = $${paramIdx++}`);
        params.push(fecha);
      }
      if (proveedor !== undefined) {
        sets.push(`proveedor = $${paramIdx++}`);
        params.push(proveedor);
      }
      if (descripcion !== undefined) {
        sets.push(`descripcion = $${paramIdx++}`);
        params.push(descripcion);
      }
      if (monto !== undefined) {
        sets.push(`monto = $${paramIdx++}`);
        params.push(monto);
      }
      if (itbms !== undefined) {
        sets.push(`itbms = $${paramIdx++}`);
        params.push(itbms);
      }
      if (monto_total !== undefined) {
        sets.push(`monto_total = $${paramIdx++}`);
        params.push(monto_total);
      }
  ...
  asyncHandler(
    async (
      ...
      if (gasto.rows[0].solicitud_reembolso_id !== null) {
        res.status(400).json({
          success: false,
          error: 'No se puede eliminar un gasto ya reembolsado',
        });
        return;
      }
  ...
  asyncHandler(
    async (req: Request<{ id: string }>, res: Response): Promise<void> => {
      ...
      if (!file) {
        res
          .status(400)
          .json({ success: false, error: 'No se proporcionó archivo' });
        return;
      }

      // Verify caja exists
      const caja = await query<{ id: number }>(
        'SELECT id FROM cajas_menudas WHERE id = $1',
        [id],
      );
      if (caja.rows.length === 0) {
        res
      ...
      res.status(201).json({
        success: true,
        data: { id: result.rows[0].id, nombre_original: file.originalname },
      });
    },
  ),
);
  ...
  asyncHandler(
    async (req: Request<{ id: string }>, res: Response): Promise<void> => {
      ...
      if (
        ...
        !result.rows[0].comprobante_apertura_r2_key
      ) {
        res.status(404).json({
          success: false,
          error: 'Comprobante de apertura no encontrado',
        });
        return;
      }
      ...
      res.json({
        success: true,
        data: { url, nombre: result.rows[0].comprobante_apertura_nombre },
      });
    },
  ),
);
...
router.get(
  ...
  asyncHandler(
    async (req: Request<{ id: string }>, res: Response): Promise<void> => {
      ...
      if (
        result.rows.length === 0 ||
        !result.rows[0].comprobante_cierre_r2_key
      ) {
        res.status(404).json({
          success: false,
          error: 'Comprobante de cierre no encontrado',
        });
        return;
      }
      const url = await getFileSignedUrl(
      ...
      res.json({
        success: true,
        data: { url, nombre: result.rows[0].comprobante_cierre_nombre },
      });
    },
  ),
);
...
router.post(
  ...
  asyncHandler(
    async (
      req: Request<{ id: string; historialId: string }>,
      res: Response,
    ): Promise<void> => {
      ...
      if (prevKey) {
        try {
          ...
          console.error(
            'Error deleting previous historial comprobante:',
            prevKey,
            err,
          );
        }
  ...
  asyncHandler(
    async (
      ...
      if (result.rows.length === 0 || !result.rows[0].comprobante_r2_key) {
        res
          .status(404)
          .json({ success: false, error: 'Comprobante no encontrado' });
        return;
      }
      const url = await getFileSignedUrl(result.rows[0].comprobante_r2_key);
      res.json({
        success: true,
        data: { url, nombre: result.rows[0].comprobante_nombre },
      });
    },
  ),
);
  ...
  asyncHandler(
    async (
      ...
      if (result.rows.length === 0) {
        res
          .status(404)
          .json({ success: false, error: 'Adjunto no encontrado' });
        return;
      }
      ...
      res.json({
        success: true,
        data: { url, nombre: result.rows[0].nombre_original },
      });
    },
  ),
);
...
router.delete(
  ...
  asyncHandler(
    async (
      ...
      await query('DELETE FROM cajas_menudas_adjuntos WHERE id = $1', [
        adjuntoId,
      ]);
    ...
    async (req: Request<{ id: string }>, res: Response): Promise<void> => {
      ...
      try {
        ...
        if (gastosResult.rows.length === 0) {
          ...
          res.status(400).json({
            success: false,
            error: 'No hay gastos pendientes de reembolso',
          });
          return;
        }

        // Calculate total and date range
        const total = gastosResult.rows.reduce(
          (sum, g) => sum + Number(g.monto_total),
          0,
        );
            ...
            for (const adj of pendingAdjuntos.rows) {
              try {
                ...
                if (adj.tipo_mime === 'application/pdf') {
                  ...
                  adj.tipo_mime === 'image/jpeg' ||
                  adj.tipo_mime === 'image/png'
                ) {
                  const img =
                    adj.tipo_mime === 'image/jpeg'
                ...
                console.error(
                  `Error procesando adjunto ${adj.nombre_original}:`,
                  err,
                );
              }
            }
            ...
            if (mergedPdf.getPageCount() > 0) {
              ...
              await query(
               ...
               VALUES ($1, $2, $3, 'application/pdf', $4, $5)`,
                [
                  solicitudId,
                  `Adjuntos Caja Menuda - ${caja.nombre}.pdf`,
                  mergedKey,
                  pdfBuffer.length,
                  user.id,
                ],
