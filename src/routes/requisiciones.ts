...
type RequisicionEstado = 'pendiente' | 'en_cotizacion' | 'por_aprobar' | 'aprobada' | 'pagada' | 'rechazada';
...
], asyncHandler(async (req: Request<{ projectId: string }, object, object, { estado?: string; archivadas?: string }>, res: Response): Promise<void> => {
...
router.get('/:id', authenticateToken, [
  ...
  if (result.rows.length === 0) {
    res.status(404).json({ success: false, message: 'Requisición no encontrada' });
    return;
  }
...
router.post('/', authenticateToken, [
  body('numero').trim().notEmpty().withMessage('Número de requisición requerido'),
  ...
  body('items.*.descripcion').trim().notEmpty().withMessage('Descripción del item requerida'),
  body('items.*.cantidad').isNumeric().withMessage('Cantidad debe ser un número'),
  body('items.*.precio_unitario').isNumeric().withMessage('Precio unitario debe ser un número')
], asyncHandler(async (req: Request<object, object, CreateRequisicionBody>, res: Response): Promise<void> => {
  ...
  const { numero, project_id, proveedor, concepto, fecha, pdf_url, pdf_nombre, items, solicitante_id } = req.body;
  ...
  const result = await query<RequisicionRow>(`
    ...
    RETURNING *
  `, [numero, project_id, proveedor, concepto || null, fecha || new Date(), pdf_url || null, pdf_nombre || null, req.user!.id, solicitanteIdFinal, subtotalGeneral, itbmsGeneral, montoTotal]);
  ...
  res.status(201).json({ success: true, message: 'Requisición creada exitosamente', requisicion: result.rows[0] });
}, {
...
router.put('/:id', authenticateToken, [
...
], asyncHandler(async (req: Request<{ id: string }, object, { proveedor?: string; concepto?: string; items?: CreateItemBody[] }>, res: Response): Promise<void> => {
  const { id } = req.params;
  const { proveedor, concepto, items } = req.body;

  const existing = await query<RequisicionRow>('SELECT * FROM requisiciones WHERE id = $1', [id]);
  ...
  if (!['pendiente', 'en_cotizacion'].includes(existing.rows[0].estado)) {
    res.status(400).json({ success: false, message: 'Solo se pueden editar requisiciones pendientes o en cotización' });
    return;
  }
  ...
  res.json({ success: true, message: 'Requisición actualizada', requisicion: result.rows[0] });
}));

// Cambiar estado
router.patch('/:id/estado', authenticateToken, [
  param('id').isInt().withMessage('ID de requisición inválido'),
  body('estado').isIn(['pendiente', 'en_cotizacion', 'por_aprobar', 'aprobada', 'pagada', 'rechazada']).withMessage('Estado inválido'),
  body('comentario').optional().trim()
], asyncHandler(async (req: Request<{ id: string }, object, { estado: RequisicionEstado; comentario?: string }>, res: Response): Promise<void> => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    res.status(400).json({ success: false, message: 'Datos inválidos', errors: errors.array() });
    return;
  ...
  if (!transicionesPermitidas[estadoAnterior].includes(estado)) {
    res.status(400).json({ success: false, message: `No se puede cambiar de "${estadoAnterior}" a "${estado}"` });
    return;
  }
  ...
  if (estado === 'pagada') {
    ...
    const gastoResult = await query<{ id: number }>(`
      ...
      RETURNING id
    `, [requisicion.project_id, `Requisición ${requisicion.numero}: ${requisicion.concepto || requisicion.proveedor}`, requisicion.monto_total, req.user!.id]);
  ...
  res.json({ success: true, message: `Requisición cambiada a "${estado}"`, requisicion: result.rows[0] });
}));
...
router.patch('/:id/archivar', authenticateToken, requireManager, [
  ...
  if (existing.rows[0].archivada) {
    res.status(400).json({ success: false, message: 'La requisición ya está archivada' });
    return;
  }
  ...
  res.json({ success: true, message: 'Requisición archivada exitosamente' });
}));
...
router.patch('/:id/restaurar', authenticateToken, requireManager, [
  ...
  if (!existing.rows[0].archivada) {
    res.status(400).json({ success: false, message: 'La requisición no está archivada' });
    return;
  }
  ...
  res.json({ success: true, message: 'Requisición restaurada exitosamente' });
}));
...
router.delete('/:id', authenticateToken, requireManager, [
  ...
  res.json({ success: true, message: 'Requisición eliminada permanentemente' });
}));

// Buscar items
router.get('/items/search', authenticateToken, asyncHandler(async (req: Request<object, object, object, { q?: string; project_id?: string; categoria_id?: string }>, res: Response): Promise<void> => {
  ...
  const result = await query<RequisicionItemRow & { requisicion_numero: string; proveedor: string; requisicion_estado: string; requisicion_fecha: Date; proyecto_nombre: string; proyecto_corto: string }>(`
