...
router.post(
  ...
  [
    body('proyecto_id')
      .isInt()
      .withMessage('ID de proyecto debe ser un número'),
    body('tipo')
      .isIn(['tiempo', 'costo', 'mixta'])
      .withMessage('Tipo debe ser tiempo, costo o mixta'),
    authenticateToken,
    requireManager,
  ],
  asyncHandler(
    async (
      req: Request<object, object, CreateAdendaBody>,
      res: Response,
    ): Promise<void> => {
      ...
      const projectCheck = await query<{ id: number }>(
...
router.put(
  ...
  [
    ...
    body('estado')
      .optional()
      .isIn(['en_proceso', 'aprobada', 'rechazada'])
  ...
  asyncHandler(
    async (
      req: Request<{ id: string }, object, UpdateAdendaBody>,
      res: Response,
    ): Promise<void> => {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
      ...
      const existingAdenda = await query<AdendaRow>(
        'SELECT * FROM adendas WHERE id = $1',
        [id],
      );
      ...
      const fechaAprobacionFinal =
        estado === 'aprobada' && !existingAdenda.rows[0].fecha_aprobacion
