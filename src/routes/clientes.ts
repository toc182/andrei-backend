...
router.post(
  '/',
  authenticateToken,
  checkPermission('clientes_agregar'),
  asyncHandler(
    async (
      req: Request<object, object, CreateClientBody>,
      res: Response,
    ): Promise<void> => {
      const {
        nombre,
...
router.put(
  '/:id',
  authenticateToken,
  checkPermission('clientes_editar'),
  asyncHandler(
    async (
      req: Request<{ id: string }, object, CreateClientBody>,
      res: Response,
    ): Promise<void> => {
      const { id } = req.params;
      const { nombre, abreviatura, contacto, telefono, email, direccion } =
        req.body;
  ...
  `,
      ...
      if (asignacionesCheck.rows.length > 0) {
        res.status(400).json({
          success: false,
          message:
            'No se puede eliminar el cliente porque tiene asignaciones de equipos',
        });
        return;
      }
